    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnConfirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String name = "";
                String dis ="0", num="0";
                name = voucherName.getText().toString();
                dis = voucherDis.getText().toString();

                num = voucherNum.getText().toString();

                String id = databaseUsers.push().getKey();

                if(!name.isEmpty() && !dis.isEmpty() && !num.isEmpty()){
                    Double disdouble = Double.parseDouble(dis);
                            ...
                            databaseUsers.child("voucher").child(name).setValue(voucher).addOnCompleteListener(new OnCompleteListener<Void>() {
                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()){
                                        ...
                                        startActivity(new Intent(AddVoucher.this, ChangeVoucher.class));
                                    }
                                }
                            });
                        }
                        else {
                            Toast.makeText(AddVoucher.this, "Please Enter amount more than 0", Toast.LENGTH_SHORT).show();
                        }

                    }else{
                        Toast.makeText(AddVoucher.this, "Please Enter discount between 0 and 100", Toast.LENGTH_SHORT).show();

                    }
                }else {
                    Toast.makeText(AddVoucher.this, "Please Enter all information", Toast.LENGTH_SHORT).show();
                }
